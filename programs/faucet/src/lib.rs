...
pub struct InitializeFaucet<'info> {
    #[account(init)]
    faucet_config: ProgramAccount<'info, FaucetConfig>, // 新 account 需要初始化数据

    #[account("token_program.key == &token::ID")] //检查token_program是否正确
    token_program: AccountInfo<'info>,

    #[account(mut)]
    token_mint: AccountInfo<'info>, //让 Program能够把数据持久化到 account.data 中
